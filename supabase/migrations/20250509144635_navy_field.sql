/*
  ...
  1. Changes
    - Update trigger function to handle admin status
    - Add policy for admin status changes
    - Update existing profiles
  
  2. Security
    - Only admins can modify admin status
    - Users can't change their own admin status
*/
...
CREATE POLICY "Only admins can set admin status"
  ...
  WITH CHECK (
    -- Users can't change their own admin status
    (id != auth.uid()) OR
    -- Non-admins can't set admin status
    (EXISTS (
      SELECT 1 FROM public.profiles
      WHERE id = auth.uid() AND is_admin = true
    ))
