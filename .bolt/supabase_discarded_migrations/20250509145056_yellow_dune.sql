/*
  ...
  1. Changes
    ...
    - Create test users with correct metadata
...
DROP POLICY IF EXISTS "Only admins can set admin status" ON public.profiles;
...
CREATE POLICY "Users can update own profile"
  ...
  USING (auth.uid() = id)
  WITH CHECK (
    (auth.uid() = id AND is_admin IS NOT DISTINCT FROM OLD.is_admin) -- Users can't change their own admin status
    OR 
    (EXISTS (
      SELECT 1 FROM public.profiles
      WHERE id = auth.uid() AND is_admin = true
    )) -- Only admins can change other profiles
  );
